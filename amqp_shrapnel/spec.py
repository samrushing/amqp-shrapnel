...
# generated by util/codegen.py from /Users/rushing/src/rabbitmq-codegen-default/amqp-rabbitmq-0.9.1.json
from wire import *
...
class access (object):
    class request (object):
        ...
        def pack (self):
            ...
            if self.exclusive: byte |= (1<<0)
            if self.passive: byte |= (1<<1)
            if self.active: byte |= (1<<2)
            if self.write: byte |= (1<<3)
            if self.read: byte |= (1<<4)
            r.append (chr (byte))
            return "".join (r)
...
class exchange (object):
    class declare (object):
        ...
        def pack (self):
            ...
            if self.passive: byte |= (1<<0)
            if self.durable: byte |= (1<<1)
            if self.auto_delete: byte |= (1<<2)
            if self.internal: byte |= (1<<3)
            if self.nowait: byte |= (1<<4)
            r.append (chr (byte))
            byte = 0
    ...
    class delete (object):
        ...
        def pack (self):
            ...
            r.append (pack_short (self.ticket))
            r.append (pack_shortstr (self.exchange))
            if self.if_unused: byte |= (1<<0)
            if self.nowait: byte |= (1<<1)
            r.append (chr (byte))
            return "".join (r)
...
class queue (object):
    class declare (object):
        ...
        def pack (self):
            ...
            if self.passive: byte |= (1<<0)
            if self.durable: byte |= (1<<1)
            if self.exclusive: byte |= (1<<2)
            if self.auto_delete: byte |= (1<<3)
            if self.nowait: byte |= (1<<4)
            r.append (chr (byte))
            byte = 0
            r.append (pack_table (self.arguments))
    ...
    class delete (object):
        ...
        def pack (self):
            ...
            r.append (pack_shortstr (self.queue))
            if self.if_unused: byte |= (1<<0)
            if self.if_empty: byte |= (1<<1)
            if self.nowait: byte |= (1<<2)
            r.append (chr (byte))
            return "".join (r)
    class delete_ok (object):
...
class basic (object):
    ...
    class consume (object):
        ...
        def pack (self):
            ...
            if self.no_local: byte |= (1<<0)
            if self.no_ack: byte |= (1<<1)
            if self.exclusive: byte |= (1<<2)
            if self.nowait: byte |= (1<<3)
            r.append (chr (byte))
            byte = 0
            r.append (pack_table (self.arguments))
            return "".join (r)
    ...
    class publish (object):
        ...
        def pack (self):
            ...
            r.append (pack_shortstr (self.exchange))
            r.append (pack_shortstr (self.routing_key))
            if self.mandatory: byte |= (1<<0)
            if self.immediate: byte |= (1<<1)
            r.append (chr (byte))
            return "".join (r)
